...
```tsx
import { Dialog } from '@snowye-ui/react';
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <span>
          The modality of the dialog. When set to `true`, interaction
    ...
    },
    {
      ...
      description: (
        <span>
          A unique identifier for the component. The `id` is used to
          generate `id` attributes for nested components. If no{' '}
          `id` prop is provided, a generated id will be used.
        </span>
      ),
    },
  ]}
  ...
  data={[
    {
      ...
      description: (
        <span>
          ...
          part, it will be inherited by `Dialog.Overlay` and{' '}
          `Dialog.Content`.
        </span>
      ),
    },
